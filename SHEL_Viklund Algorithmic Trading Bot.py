...
class MovingAverageCrossAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.shel = self.AddEquity("SHEL", Resolution.Daily, Market.USA).Symbol #Add Seclected Security
        ...
        self.SetBenchmark("QEFA") #A NASDAQ ETF that tracks companies with shares in the NASDAQ including Shell Plc


        # create a 5 day exponential moving average
        self.fast = self.EMA(self.shel, 5, 0.3)

        # create a 30 day exponential moving average
        self.slow = self.EMA(self.shel, 30, 0.8)
    ...
    def OnData(self, data):
        ...
        self.Plot("Data Chart", self.shel, self.Securities[self.shel].Close)
        ...
        if self.earnings_period_end_date is not None:
            ...
            if self.earnings_period_end_date <= self.Time:
                self.Liquidate()
                self.Log('Liquidated')
                self.Plot("Data Chart", "V-Sell", self.Securities[self.shel].Price)
        ...
        if self.earnings_call_period == False:
            ...
            if not self.viklund_control:
                ...
                if self.Portfolio["SHEL"].Quantity <= 0:
                    ...
                    if self.fast.Current.Value > self.slow.Current.Value * (1 + self.tolerance ):
                        self.SetHoldings("SHEL", 0.8)
                        self.SetHoldings("BND", -0.2)
                        self.Plot('Data Chart', "Buy", self.Securities[self.shel].Price)
                ...
                if self.Portfolio["SHEL"].Quantity > 0 and (self.fast.Current.Value * (1 + self.tolerance )) < self.slow.Current.Value:
                    self.Liquidate()
                    self.Plot("Data Chart", "Sell", self.Securities[self.shel].Price)
        ...
        # Triggering Viklan NN to take over trading decisions
            ...
            if signal > 0.2:
                ...
                # Position sizing based off NN signal, 80% if our cash into SHEL the remaining allocated towards BNDs
                self.SetHoldings("SHEL", 0.8)
                self.SetHoldings("BND", -0.2)
                self.Log('Buy')
                # Plot the price at which this buy order was placed
                self.Plot("Data Chart", "V-Buy", self.Securities[self.shel].Price)
            ...
            #If the signal is less than our certainty threshold then we go short
                ...
                self.earnings_period_end_date = self.Time + timedelta(days=(0.2 * 30))
                 # Set Viklund control flag to true to prevent core trading strategy, EMA cross, from taking over
                self.viklund_control = True
                # Position sizing based off NN signal, 80% if our cash into shorting SHEL, the remaining allocated towards BNDs
                self.SetHoldings("SHEL", -0.8)
                self.SetHoldings("BND", 0.2)
                self.Log('Short')
                # Plot the price at which this short order was placed
                self.Plot("Data Chart", "Short", self.Securities[self.shel].Price)
            # Plot the price at which this short order was placed  
    ...
    def get_text(self):
        ...
        url = 'https://www.dropbox.com/scl/fi/b6ixmavoqgts8jgvhi9va/viklund_sent_signals.csv?rlkey=8c9wxyaiij1rz09g1cavgmc8p&dl=1'
        ...
        for i in range(1,len(data)-1):
            # Date time split
            proce_date = datetime.strptime(data[i].split(',')[0], '%Y-%m-%d %H:%M:%S%z').replace(tzinfo=None)
